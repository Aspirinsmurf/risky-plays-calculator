...
## Introduction
AnyPlay is a probability calculator and risk analysis tool for Blood Bowl 2020. It uses a human-readable notation language to enable rapid input of sequences of actions. The input is text, and the output is one or more tables of action sequences and associated probabilities. It's like AnyDice for Blood Bowl, if you're familiar with that application.
...
```
> In this example, one player tries dodging out with the Dodge skill on a 4+, picking up the ball with Sure Hands on a 2+, making a 2+ roll rush, blitzing with two dice and getting either defender down, defender stumbles or a push.
...
Optionally, you can add a label to a normal roll with `"label"` before it, e.g. `"Rush" 2+` or `"Dodge" 4+`. The label is printed to the action's line in the output table. This can be useful for clarity, but it's irrelevant for calculations.
...
###  Acceptable outcomes
To determine the of acceptable outcomes, consult this chart.
...
You can either list the acceptable results out in the parantheses like `(dd,ds,bds), or you can add up their values and write the total `n` without the parentheses.
...
#### Flags
A flag is a virtual action added to a player's action sequence to account for an unusual circumstance, such as having an usual skill.

Let's say you have a Loner player (without the Dodge skill) trying to make a 4+ dodge and a 2+ dodge. This could be represented as follows:
```
loner, 4+, 3+;
```
A flag is valid for all actions that come after it in the sequence. So for the example above, using `4+, 3+, loner;` would give the wrong results.
